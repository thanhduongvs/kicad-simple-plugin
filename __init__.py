...
def check_for_bom_button():
    ...
    while True:
        ...
        if button_wx_item_id == 0 or not top_tb.FindTool(button_wx_item_id):
            ...
            top_tb.AddTool(button_wx_item_id, "LSimplePlugin", bm,
                           "Simple Plugin", wx.ITEM_NORMAL)
            top_tb.Bind(wx.EVT_TOOL, callback, id=button_wx_item_id)
            top_tb.Realize()


if not os.environ.get('SIMPLE_CLI_MODE', False):
    from .kicad.kicad import SimplePlugin
